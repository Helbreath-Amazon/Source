...
#include "Curse.h"
#include "Cint.h"
#include <tlhelp32.h> // MORLA 2.14 - Anti Hack
...
public:
	...
	void NotifyMsg_REPDGDEATHS(char * pData); // MORLA 2.2 - Actualiza DGPoints, REP, y Deaths
	void NotifyMsg_TopPlayers(char *pData); // MORLA 2.4 - Top Players
...
// Centuu
	...
	DWORD m_dwAuraTime, m_dwResurTime, m_dwVentanaTime; // morla venite
    short m_sZerk, m_sInv, m_sPfm, m_sPfa, m_sShield, m_sNpcBar; // MORLA2- NPC Bar
	int iNpcBar_HP, iNpcBar_MP;	
	void NotifyMsg_Top15(char *pData); // MORLA 2.10 - Top 15 en deathmach
	void DrawDialogBox_GMPanel_TopServer(short msX, short msY); // TOP 15 server
	void DlgBoxClick_GMPanel_TopServer(short msX, short msY); // MORLA 2.9 - Top Server
	void NotifyMsg_Top15HB(char *pData); // MORLA 2.10 - Top 15 en deathmach
	...
	void DlgBoxClick_GMPanel_TradeEk(short msX, short msY);
	void DrawDialogBox_GMPanel_TradeEk(short msX, short msY);
	void DlgBoxClick_GMPanel_TopDeathmach(short msX, short msY);
	void DrawDialogBox_GMPanel_TopDeathmach(short msX, short msY);

	void UseShortCut( int num );
	void UseShortCutPanel( int num, bool save ); // MORLA - Declaro el UseShortCutPanel
	BOOL bReadLoginConfigFile(char * cFn); // Login.cfg
	...
	int m_iGameServerMode; // Centuu : Reintroducido para el uso de Login.cfg
...
//	int m_iAccountStatus;
	...
	short m_sShortCut[11]; // Snoopy: 6 shortcuts // KaoZureS + 5 para la Cast Bar
	...
	char m_cItemOrder[DEF_MAXITEMS]; // MORLA 2.8 - bag mess
	...
	int m_cLU_Str, m_cLU_Vit, m_cLU_Dex, m_cLU_Int, m_cLU_Mag, m_cLU_Char; // morla 2.7 - para que no se pongan en negativo
   ...
   BOOL CheckProcesses(); // Morla 2.14 - Anti Hack
